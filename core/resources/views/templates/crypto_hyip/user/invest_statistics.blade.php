                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center">
                            <div>
                                ...
                                <h4 class="text--base"><sup
                                        class="top-0 fw-light me-1">{{ gs('cur_sym') }}</sup>{{ showAmount(auth()->user()->invests->sum('amount'), currencyFormat: false) }}
                                </h4>
                            </div>
                            <div>
                                <p class="mb-2 fw-bold">@lang('Total Profit')</p>
                                <h4 class="text--base"><sup
                                        class="top-0 fw-light me-1">{{ gs('cur_sym') }}</sup>{{ showAmount(auth()->user()->transactions()->where('remark', 'interest')->sum('amount'), currencyFormat: false) }}
                                </h4>
                            </div>
                        </div>
                            ...
                            <div class="invest-statistics d-flex flex-wrap justify-content-between align-items-center">
                                <div class="flex-shrink-0">
                                    @foreach ($investChart as $chart)
                                        <p class="my-2"><i
                                                class="fas fa-plane planPoint me-2"></i>{{ showAmount(($chart->investAmount / $investChart->sum('investAmount')) * 100, currencyFormat: false) }}%
