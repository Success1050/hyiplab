                                        ...
                                        <div>
                                            ...
                                            <div class="progress">
                                                <div class="progress-bar progress-bar-striped bg-success" role="progressbar"
                                                    style="width: {{ diffDatePercent($staking->created_at, $staking->end_at) }}%"
                        ...
                        <div class="form-group">
                            ...
                            <select name="duration" class="form--control form-two select2" required>
                                ...
                                @foreach ($stakings as $staking)
                                    <option value="{{ $staking->id }}" data-interest="{{ $staking->interest_percent }}">
                                        {{ $staking->days }} @lang('Days - Interest') {{ $staking->interest_percent }}%</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="form--label">@lang('Wallet')</label>
                            <select name="wallet" class="form--control form-two select2" required
                                data-minimum-results-for-search="-1">
                                <option hidden>@lang('Select One')</option>
                                <option value="deposit_wallet">
                                    @lang('Deposit Wallet - '){{ showAmount(auth()->user()->deposit_wallet) }}</option>
                                <option value="interest_wallet">
                                    @lang('Interest Wallet - '){{ showAmount(auth()->user()->interest_wallet) }}</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="form--label">@lang('Amount')
                                ({{ showAmount(gs('staking_min_amount')) . ' - ' . showAmount(gs('staking_max_amount')) }})</label>
                            <div class="input-group">
                                <input type="number" name="amount" class="form-control form--control form-two"
                                    min="0" step="any" autocomplete="off" required>
