@forelse ($pools as $pool)
    <div class="col-xxxl-3 col-lg-4 col-sm-6">
        <div class="plan-card wow fadeInUp" data-wow-delay="0.3s">
            <div class="plan-card-header">
                <p class="plan-card-title">{{ __($pool->name) }}</p>
            ...
            <div class="plan-card-body">
                <ul class="plan-card-list">
                    <li class="plan-card-item">
                        ...
                        @lang('Total Amount') {{ gs('cur_sym') }}{{ showAmount($pool->amount, currencyFormat: false) }}
                    </li>
                    <li class="plan-card-item">
                        <span class="icon"> <i class="fas fa-check"></i></span>
                ...
                <div class="remaining">
                    ...
                    <div class="progress">
                        <div class="progress-bar bg--base customWidth"
                            data-invested="{{ getAmount(($pool->invested_amount / $pool->amount) * 100) }}"
                            role="progressbar" aria-valuenow="30" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                </div>

                <div class="plan-card-btn">
                    <button data-bs-toggle="modal" data-bs-target="#poolInvestModal" data-pool_id="{{ $pool->id }}"
                        data-pool_name="{{ __($pool->name) }}"
                        class="btn--base btn w-100 poolInvestNow">@lang('Invest Now')</button>
                </div>
            </div>
        </div>
    </div>
