@forelse ($plans as $k => $data)
    <div class="@if (@$layout == 'master') col-xl-4 col-xxxl-3 col-sm-6  @else col-xl-3 col-sm-6 @endif">
        <div class="plan-card wow fadeInUp" data-wow-delay="0.3s">
            <div class="plan-card-header">
                <p class="plan-card-title">{{ __($data->name) }}</p>
                ...
                <ul class="plan-card-list">
                    ...
                    @if ($data->fixed_amount == 0)
                        ...
                        <li class="plan-card-item">
                            <span class="icon"> <i class="fas fa-check"></i></span>
                            @lang('Max')
                            {{ gs('cur_sym') }}{{ showAmount($data->maximum, currencyFormat: false) }}
                        </li>
                    @else
                        <li class="plan-card-item">
                            <span class="icon"> <i class="fas fa-check"></i></span>
                            @lang('Amount')
                            {{ gs('cur_sym') }}{{ showAmount($data->fixed_amount, currencyFormat: false) }}
                        </li>
                    @endif
                </ul>
            </div>
            <div class="plan-card-footer">
                <div class="plan-card-btn">
                    <button data-bs-toggle="modal" data-bs-target="#investModal" data-plan="{{ $data }}"
                        class="btn--base btn w-100 investModal">@lang('Invest Now')</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog @if (auth()->check()) modal-lg @endif modal-dialog-centered modal-content-bg">
        <div class="modal-content">
            ...
            <form action="{{ route('user.invest.submit') }}" method="post">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <code class="gateway-info rate-info d-none">@lang('Rate'): 1
                                        {{ gs('cur_text') }} = <span class="rate"></span> <span
                                            class="method_currency"></span></code>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" step="any" min="0"
                                            class="form--control form-control form-two" name="amount" required>
                            ...
                            <div class="col-md-6 compoundInterest">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" min="0" class="form--control form-control form-two"
                                            name="compound_interest">
                                        <div class="input-group-text input-style">@lang('Times')</div>
                                    </div>
                                    <small class="fst-italic text--info"><i class="las la-info-circle"></i>
                                        @lang('Your interest will add to the investment capital amount for a specific time that you\'re entering.')</small>
                                </div>
                            </div>
                            @if (gs('schedule_invest'))
                                <div class="col-md-6 investTime">
                                    <div class="form-group">
                                        ...
                                        <select class="form--control select2" name="invest_time" required
                                            data-minimum-results-for-search="-1">
                                            <option value="invest_now">@lang('Invest Now')</option>
                                            <option value="schedule">@lang('Schedule')</option>
                                        </select>
                                        <small class="fst-italic text--info"><i class="las la-info-circle"></i>
                                            @lang('You can set your investment as a scheduler or invest instant.')</small>
                                    </div>
                                </div>
                            @endif
                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="number" min="0"
                                                class="form--control form-control form-two" name="schedule_times">
                                            <span class="input-group-text  input-style">@lang('Times')</span>
                                        </div>
                                        <small class="fst-italic text--info"><i class="las la-info-circle"></i>
                                            @lang('Set how many times you want to invest.')</small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="number" class="form--control form-control form-two"
                                                name="hours" min="0">
                                            <span class="input-group-text input-style">@lang('Hours')</span>
                                        </div>
                                        <small class="fst-italic text--info"><i class="las la-info-circle"></i>
                                            @lang('Set a frequency at which you prefer to make investments.')</small>
                                    </div>
                                </div>
                            </div>
                        @endif
                ...
                @else 
                <div class="modal-body">
                    @lang('Please sign in to your account to buy a plan and start investing.')
                </div>
                @endif
                <div class="modal-footer">
                    @if (auth()->check())
                        <button type="button" class="btn btn--dark btn-dark"
                            data-bs-dismiss="modal">@lang('No')</button>
                        <button type="submit" class="btn btn--base">@lang('Yes')</button>
                    @else
                        <a href="{{ route('user.login') }}"
                            class="btn btn--base text-center">@lang('Login')</a>
                    @endif
                </div>
            </form>
        </div>
            ...
            $('.investModal').click(function() {
                ...
                if (plan.lifetime == '0') {
                    modal.find('.interestValidity').html(
                        `<strong>  Per ${plan.time_setting.name} for  ${plan.repeat_time} times</strong>`);
                } else {
                    modal.find('.interestValidity').html(
                        `<strong>  Per ${plan.time_setting.time} hours for life time </strong>`);
                }
