...
@section('content')
                ...
                <table class="table table--responsive--xl">
                    ...
                    <tbody>
                        @forelse($scheduleInvests as $scheduleInvest)
                            ...
                            @endphp
                            <tr>
                                <td>{{ __($scheduleInvest->plan->name) }} <br> {{ showAmount($scheduleInvest->amount) }}
                                </td>
                                <td>
                                ...
                                <td>{{ $scheduleInvest->next_invest ? showDateTime($scheduleInvest->next_invest) : '----' }}
                                </td>
                                <td>
                                    <div class="action--btns d-flex gap-2 justify-content-end">
                                        @if ($scheduleInvest->rem_schedule_times)
                                            @if ($scheduleInvest->status)
                                                <button class="btn btn-outline--dark btn--sm confirmationBtn"
                                                    data-question="@lang('Are you sure to pause this schedule invest?')"
                                                    data-action="{{ route('user.invest.schedule.status', $scheduleInvest->id) }}">
                                                    <i class="fas fa-pause"></i>
                                                </button>
                                            @else
                                                <button class="btn btn-outline--success btn--sm confirmationBtn"
                                                    data-question="@lang('Are you sure to continue this schedule invest?')"
                                                    data-action="{{ route('user.invest.schedule.status', $scheduleInvest->id) }}">
                                                    <i class="fas fa-play"></i>
                                                </button>
                                            @endif
                                        @endif
                                        <button class="btn btn-outline--base btn--sm detailsBtn"
                                            data-schedule_invest="{{ $scheduleInvest }}"
                                            data-interest="{{ getAmount($interest) }}"
    ...
    <x-confirmation-modal closeBtn="btn btn--dark" submitBtn="btn btn--base" />
@endsection
